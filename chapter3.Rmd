...
summary(m)
```
The model summary shows a high significance for the correlation to *sex* and *goout*, a medium significance for *famrel* und no signifant value for *failure*. One could fit the model again, dropping at least the *failure* variable. But first let's study the odds ratios.
```{r}
coef(m)
